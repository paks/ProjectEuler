...
namespace Primegen {
	...
	unsigned long long Primes::next()
	{
		return primegen_next(pg);
	}

	unsigned long long Primes::peek()
	{
		return primegen_peek(pg);;
	}

	unsigned long long Primes::count(unsigned long long to)
	{
		return primegen_count(pg,to);;
	}

	void Primes::skipto(unsigned long long to)
	{
	...
	/*TODO: inplement miller-rabin
	Boolean Primes::isPrime(unsigned long long p, int witnesses)
	{
